...
import QqmusicIcon from '@site/static/icons/qqmusic.svg'
    ...
    <animated.div className={styles.hero}>
      <div className={styles.bloghome__intro}>
        <animated.div style={animatedTexts[0]} className={styles.hero_text}>
          Hello! 我是
          <span className={styles.intro__name}>鲸语</span>
        </animated.div>
...
export function SocialLinks({ animatedProps, ...props }) {
  ...
  const socials = themeConfig.socials as { github: string; twitter: string; juejin: string; csdn: string; qq: string; wx: string; qqmusic: string}

  return (
    <animated.div className={styles.social__links} style={animatedProps}>
      <a href='https://github.com/lag-spike' target='_blank'>
        <GithubIcon />
      </a>
      <a href='https://blog.csdn.net/weixin_44113601' target='_blank'>
        <CsdnIcon />
      </a>
      <a href={socials.qq} target='_blank'>
        <QqIcon />
      </a>
      <a href='https://y.qq.com/n/ryqq/profile/like/song?uin=oKCloe-5oev5on**' target='_blank'>
        <QqmusicIcon />
      </a>
    </animated.div>
  )
